...
while keep_running:
    ...
    qdot = np.array(qdot)
    #B_to = np.array(B_to)
    Pc = np.dot(np.transpose(tau - B_full_to @ qdot), qdot) - 1
    ...
    Pc_cart = np.dot(Wt - B_scaled @ v_act_scaled, v_act_scaled) 
    ...
    for i in range(6):
        if H_n_cart[i] > epsilon:
            ...
            u_n_cart[i] = 0
    #if H_n_total > H_n_total_start * 2:
        #u_n = np.zeros(6)
        #u_n_cart = np.zeros(6)
        #s_n = [0, 0, 0, 0, 0, 0]
    tau_on = -u_n * s_n
...
Plot2 = False # Energy tanks - cartesian
Plot3 = False  # Energy tanks - joint space (bad)
PlotFriction = False
...
if Plot1:
    ...
    plt.xlabel('Time (s)')
    plt.ylabel('Power (W)')
    plt.title('Cartesian Power')
    plt.grid()
    plt.legend()
    plt.tight_layout()

    plt.figure(figsize=(8, 5))
    plt.plot(time, Pc_log, 'g-')
    plt.axhline(Pmax, color='k', linestyle=':', label='Pmax')
    plt.xlabel('Time (s)')
    plt.ylabel('Power (W)')
    plt.title('Power')
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(time, beta_log, 'm-')
    plt.xlabel('Time (s)')
    plt.ylabel('Joint Damping scaling Factor')
    plt.title('Beta')
    plt.grid()
    plt.tight_layout()
